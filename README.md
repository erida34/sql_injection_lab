...
## 5) Практика: SQL‑инъекции через POST (форма добавления) 

### 5.1 Нормальное добавление

1. Добавьте товар обычными значениями.
2. Проверьте, что он появился в таблице.

### 5.2 Ошибка при специальном вводе

1. Введите спецсимволы SQL в одно из полей так, чтобы INSERT “сломался”.
2. Зафиксируйте сообщение об ошибке.

### 5.3 “Вычисляемое значение” вместо ожидаемого (если уязвимость позволяет)

1. Если поле `price` подставляется в SQL без кавычек, проверьте, удаётся ли заставить БД вычислить выражение и сохранить результат.
2. Зафиксируйте отличие.

---

## 6) Исправление

### 6.1 Параметризованный SELECT

* Перепишите поиск на placeholders `?` в sqlite3.
* Проверьте, что “вредный” ввод больше не ломает запрос и не расширяет выдачу.

### 6.2 Параметризованный INSERT

* Перепишите добавление товара на `VALUES (?, ?, ?)`.
* Проверьте, что спецсимволы сохраняются как текст и не влияют на структуру SQL.
